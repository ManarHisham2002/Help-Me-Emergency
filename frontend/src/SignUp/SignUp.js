                            ...
                            <div id="multi-step-form-container" className="col-lg-12 col-md-12 col-sm-12">
                                <ul className="form-stepper form-stepper-horizontal text-center mx-auto pl-0 col-lg-12 col-md-12 col-sm-12">
                                    {steps.map(({ step, icon: Icon, label }) => (
                                        <li key={step} className={`form-stepper-list ${currentStep === step ? 'form-stepper-active' : currentStep > step ? 'form-stepper-completed' : 'form-stepper-unfinished'}`} step={step}>
                                ...
                                {infoUser.err && infoUser.err.length > 0 && infoUser.err !== 'This Password is Wrong' && (
                                    <div className='messageError'>
                                        <FontAwesomeIcon icon={faTriangleExclamation} className='iconMessageError' />
                                        <p className="errorMassageFrom">{infoUser.err}</p>
                                    </div>
                                )}
                                <form id="userAccountSetupForm" className="signupForm col-12" name="userAccountSetupForm" encType="multipart/form-data" method="POST">
                                    
                                    {currentStep === 1 && <Step1 navigateToFormStep={navigateToFormStep} setInfoUser={setInfoUser} infoUser={infoUser} />}
                                    {currentStep === 2 && <Step2 navigateToFormStep={navigateToFormStep} handleCheckboxChange={handleCheckboxChange} />}
                                    {currentStep === 3 && <Step3 navigateToFormStep={navigateToFormStep} handleCheckboxChange={handleCheckboxChange} />}
                                    {currentStep === 4 && <Step4 navigateToFormStep={navigateToFormStep} handleCheckboxChange={handleCheckboxChange} handleSubmit={handleSubmit} />}
                                </form>
                            </div>
                        </div>
                    </div>
